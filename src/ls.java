public class ls implements Command {
	...
	public int execute(String[] arg, Bash bash, String comanda) {
		// Comanda ls afiseaza continutul unui director sau un singur fisier, in
		// functie de path-ul specificat
		...
		if (n == 0) {
			if (Path[Path.length - 1].equals(".") == false) {
				...
				if (auxbash.getWorkingDir().hasFile(file.getName()) != null) {
					if (auxbash.getWorkingDir().hasFile(file.getName()).isfile == false
							&& auxbash.getWorkingDir().hasFile(file.getName())
									.hasxPermision(bash.getUser()) == false) {
