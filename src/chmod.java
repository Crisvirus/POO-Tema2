public class chmod implements Command {
	...
	public int execute(String[] arg, Bash bash, String comanda) {
		// Comanda chmod schimba permisiunile unui director sau ale unui fisier
		...
		if (n == 0) {
			...
			if (auxbash.getWorkingDir().hasFile(dir.getName()) != null) {
				if (auxbash.getWorkingDir().hasFile(dir.getName())
						.haswPermision(auxbash.getUser())) {
					auxbash.getWorkingDir().hasFile(dir.getName())
							.setPermisiuni(p);
				} else {
